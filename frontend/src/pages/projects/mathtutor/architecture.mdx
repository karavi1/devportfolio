...
flowchart LR
  ...
  subgraph "Model Layer"
    ...
    oss_llm["Self-Hosted LLM (optional)"]
  end
...
`} />
      ...
      <div className="text-sm mt-2">
        The current repo is a POC centered on <b>problem suggestion</b> using an <b>obstacle</b> chosen by the user, a <b>structured prompt</b>, and <b>difficulty classification</b>. The API layer can stay thin while the orchestration and schemas live server-side.
      </div>
...
`} />
      ...
      <div className="text-sm mt-2">
        Suggested constraints: <code>unique(problem_id, step_no)</code> on <b>HINT</b>; <code>unique(problem_id, topic_id)</code> on <b>PROBLEM_TAG</b>.
      </div>
...
sequenceDiagram
  ...
  FE-->>U: Display hint; user continues work
`} />
...
sequenceDiagram
  ...
  API->>LLM: Evaluate via rubric (compare to canonical; check steps)
  LLM-->>API: {is_correct, feedback_text, next_hint?, confidence, rubric_json}
  ...
  FE-->>U: Show result; offer retry or next problem
`} />
...
`} />
      ...
      <ul className="text-sm mt-2">
        ...
        <li><b>FastAPI</b>: Orchestrates structured prompts + JSON schemas; adapters for hosted or self-hosted LLMs.</li>
        <li><b>RDS MySQL</b>: Stores problems, hints, attempts, evaluations, and classifications.</li>
        <li><b>Vector DB (optional)</b>: Retrieve similar examples or formulas for context.</li>
      ...
      <div className="text-sm mt-4">
        Implementation notes: keep <b>Pydantic schemas</b> for structured outputs; enforce <b>transactions</b> when creating problems with hints/classification; prefer deterministic JSON outputs from the LLM for reliability.
      </div>
